...
export class ManageService {
    ...
    public async getActors(): Promise<Actor[] | boolean> {
        return (
            this.http.get<Actor[]>(`${API_URL}actors`,
                                   { headers: new HttpHeaders().set("Authorization", localStorage.getItem(Token.id) as unknown as string) })
                .toPromise()
        );
    }

    public deleteMovie(idMovie: number): Observable<HTTP> {
    ...
    public addParticipant(toAdd: Participant, movieID: number): Observable<HTTP> {
        return (
            this.http.post<HTTP>(`${API_URL}participants/insert`,
                                 {
                    name: toAdd.name,
    ...
    public addParticipation(participantID: number, role: string, salary: number, movieID: number): Observable<HTTP> {
        return (
            this.http.post<HTTP>(`${API_URL}participation/insert`,
                                 {
    ...
    public addCeremony(toAdd: Oscar, movieID: number): Observable<HTTP> {
        return (
            this.http.post<HTTP>(`${API_URL}ceremony/insert`,
                                 {
                    date: toAdd.date,
    ...
    public addUser(toAdd: User): Observable<HTTP> {
        return (
            this.http.post<HTTP>(`${API_URL}users/insert`,
                                 toAdd,
                                 {
                    headers: new HttpHeaders().set("Authorization",
                                                   localStorage.getItem(Token.id) as unknown as string)
